...
# Storage server
STORAGE_SERVER=${PROJECT_PATH}/src/P2-f12/official/storageserver/storageserver

# Libclient
LIBCLIENT=${PROJECT_PATH}/bin/linux_amd64/libclient
...
function startStorageServers {
    ...
    then
        for i in `seq 1 $((N-1))`
        do
            ${STORAGE_SERVER} -id=${STORAGE_ID[$i]} -master="localhost:${STORAGE_PORT}" 2> /dev/null &
...
function testRouting {
    ...
    for KEY in "${KEYS[@]}"
    do
        ${LIBCLIENT} -port=${STORAGE_PORT} p ${KEY} value > /dev/null
        PASS=`${LIBCLIENT} -port=${STORAGE_PORT} g ${KEY} | grep value | wc -l`
        if [ "$PASS" -ne 1 ]
        then
            break
        fi
...
function testDelayedStart {
    ...
    ${STORAGE_SERVER} -master="localhost:${STORAGE_PORT}" 2> /dev/null &
    STORAGE_SERVER_PID2=$!
    sleep 5
